...
def ea_prompt(ea_id, title, scope, l1):
    ...
    return (
        ...
        "  subgraph closes.\n"
        "- EVERY classDef and class statement goes on ITS OWN LINE, in a block AFTER the\n"
        "  last subgraph closes. NEVER put a classDef on the same line as a node — that\n"
        "  is a hard mermaid parse error. Wrong: NODE[\"label\"] classDef sparse fill:...\n"
        "  Right: NODE[\"label\"] on its own line, then later classDef sparse fill:... on\n"
        "  its own line, then class NODE sparse on its own line.\n"
        ...
        "  for undocumented layers include these two lines, each on its own line:\n"
        "  classDef sparse fill:#f8fafc,stroke:#94a3b8,color:#64748b,stroke-dasharray: 6 4\n"
        "  class <the sparse node ids, comma separated> sparse\n"
